...
type rbdDriver int32

const (
	noDriver   rbdDriver = 1
	krbdDriver rbdDriver = 1 << iota
	nbdDriver  rbdDriver = 1 << iota
)

// find any device to which this device is mapped
func getDevFromImageAndPool(pool, image string) (string, bool) {
  device, found := getRbdDevFromImageAndPool(pool, string)
  if (!found) {
    device, found := getNbdDevFromImageAndPool(pool, string)
  }
  return device, found
}
...
// figure out any existing rbd-nbd process mapping this pool/image
// by finding and parsing the command-line of process
// associated with each nbd device
func getNbdDevFromImageAndPool(pool, image string) (string, bool) {
	...
	for i := 0; ; i++ {
		...
		if err != nil {
			glog.V(4).Infof("Did not find valid pid file in dir %s: %v", nbd_path, err)
			continue
		}
		...
		for n := 3; n < len(cmdLineArgs)-1; n++ {
			if cmdLineArgs[n] == "--pool" {
				if cmdLineArgs[n+1] == pool {
					glog.V(4).Infof("found rbd-nbd device mapped to pool %s, image %s",
						pool, image)
					return path.Join("/dev", "nbd"+strconv.Itoa(i)), true
				}
			}
		}
	}
...
func waitForPath(pool, image string, maxRetries int, drivers rbdDriver) (string, bool) {
	for i := 0; i < maxRetries; i++ {
		if (drivers & krbdDriver) != 0 {
			if devicePath, found := getRbdDevFromImageAndPool(pool, image); found {
				return devicePath, true
			}
		}
		if (drivers & nbdDriver) != 0 {
...
func execMapCommand(rbdCmd string, mon string, b rbdMounter) ([]byte, error) {
	glog.V(4).Infof("using %s for map function", rbdCmd)
	if b.Secret != "" {
...
func (util *RBDUtil) AttachDisk(b rbdMounter) (string, error) {
	...
	if err != nil {
		...
		glog.V(5).Infof("rbd-nbd tools are not found.")
	} else {
		glog.V(4).Infof("rbd-nbd tools are found. Will try to map use rbd-nbd")
		drivers |= nbdDriver
	}

	devicePath, found := waitForPath(b.Pool, b.Image, 1, drivers)
	if !found {
		...
		if mapDriver == noDriver && (drivers|nbdDriver) != 0 {
			output, err = execMapCommand("rbd-nbd", mon, b)
			if err != nil {
				glog.Warningf("rbd-nbd: falling back to krbd after rbd-nbd error %v, output %s",
					err, string(output))
			} else {
				mapDriver = nbdDriver
			}
		}
		if mapDriver == noDriver && (drivers|krbdDriver) != 0 {
			_, err = b.exec.Run("modprobe", "rbd")
			if err != nil {
				glog.Warningf("rbd: failed to load rbd kernel module: %v", err)
			}
			output, err = execMapCommand("rbd", mon, b)
			if err != nil {
				glog.Warningf("rbd-nbd: falling back to krbd after rbd-nbd error %v, output %s",
					err, string(output))
			} else {
				mapDriver = krbdDriver
			}
		}
		if mapDriver == noDriver {
			return "", fmt.Errorf("None of the available drivers could map image %s/%s", b.Pool, b.Image)
		}

		devicePath, found = waitForPath(b.Pool, b.Image, 10, mapDriver)
		if !found {
